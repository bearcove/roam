...
impl ChannelCore {
    fn new() -> Self {
        Self {
            ...
            binding_changed: Notify::new("telex_types.channel.binding_changed"),
        }
    }
    ...
    pub fn bind_retryable_receiver(self: &Arc<Self>, bound: BoundChannelReceiver) {
        ...
        let state = guard.get_or_insert_with(|| {
            let (tx, rx) = mpsc::channel("telex_types.channel.logical_receiver", 64);
                    ...
                    Some(LogicalIncomingChannelMessage {
                        ...
                        let value = msg
                            .try_repack(|item, _backing_bytes| {
                                ...
                                telex_postcard::from_slice_borrowed(bytes)
...
pub enum RxError {
    Unbound,
    Reset,
    Deserialize(telex_postcard::error::DeserializeError),
    Protocol(String),
}
...
mod tests {
    ...
    async fn rx_recv_returns_none_on_close() {
        let (tx, rx_inner) = mpsc::channel("telex_types.channel.test.rx1", 1);
        let mut rx = Rx::<u32>::unbound();
        rx.bind(rx_inner);
    ...
    async fn rx_recv_returns_reset_error() {
        let (tx, rx_inner) = mpsc::channel("telex_types.channel.test.rx2", 1);
        let mut rx = Rx::<u32>::unbound();
        rx.bind(rx_inner);
    ...
    async fn rx_recv_rejects_outgoing_payload_variant_as_protocol_error() {
        ...
        let (tx, rx_inner) = mpsc::channel("telex_types.channel.test.rx3", 1);
        let mut rx = Rx::<u32>::unbound();
        rx.bind(rx_inner);
    ...
    async fn rx_recv_notifies_replenisher_after_consuming_an_item() {
        let (tx, rx_inner) = mpsc::channel("telex_types.channel.test.rx4", 1);
        let replenisher = Arc::new(CountingReplenisher::new());
        let mut rx = Rx::<u32>::unbound();
        rx.bind(rx_inner);
        rx.replenisher.inner = Some(replenisher.clone());

        let encoded = telex_postcard::to_vec(&123_u32).expect("serialize test item");
        let item = SelfRef::owning(
            Backing::Boxed(Box::<[u8]>::default()),
            ChannelItem {
    ...
    async fn rx_recv_logical_receiver_decodes_items_and_notifies_replenisher() {
        let (tx, rx_inner) = mpsc::channel("telex_types.channel.test.rx5", 1);
        let replenisher = Arc::new(CountingReplenisher::new());
        let core = Arc::new(ChannelCore::new());
        ...
        let encoded = telex_postcard::to_vec(&321_u32).expect("serialize test item");
        let item = SelfRef::owning(
            Backing::Boxed(Box::<[u8]>::default()),
            ChannelItem {
                item: Payload::PostcardBytes(Box::leak(encoded.into_boxed_slice())),
    ...
    impl ChannelBinder for TestBinder {
        ...
        fn create_rx(&self) -> (ChannelId, BoundChannelReceiver) {
            let (tx, rx) = mpsc::channel("telex_types.channel.test.bind_retryable1", 8);
        ...
        fn register_rx(&self, _channel_id: ChannelId) -> BoundChannelReceiver {
            let (_tx, rx) = mpsc::channel("telex_types.channel.test.bind_retryable2", 8);
            BoundChannelReceiver {
                receiver: rx,
                liveness: None,
                replenisher: None,
    ...
    async fn case1_serialize_tx_allocates_and_binds_paired_rx() {
        ...
        let bytes =
            with_channel_binder(&binder, || telex_postcard::to_vec(&args).expect("serialize"));
    ...
    fn case3_deserialize_tx_binds_via_binder() {
        ...
        let mut bytes = telex_postcard::to_vec(&42_u32).unwrap();
        bytes.extend_from_slice(&telex_postcard::to_vec(&ChannelId(7)).unwrap());

        let binder = TestBinder::new();
        let args: Args = with_channel_binder(&binder, || {
            telex_postcard::from_slice(&bytes).expect("deserialize")
        });
    ...
    fn channel_id_round_trips_through_ser_deser() {
        ...
        let bytes = with_channel_binder(&caller_binder, || {
            telex_postcard::to_vec(&args).expect("serialize")
        });
