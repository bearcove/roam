...
use crate::{
    ...
    MaybeSendFuture, MaybeSync, Metadata, RequestCall, RequestResponse, TelexError, SelfRef,
    ServiceDescriptor,
};
...
pub type CallResult = Result<crate::WithTracker<SelfRef<RequestResponse<'static>>>, TelexError>;
...
//
// impl HashClient {
//   async fn hash(&self, payload: &[u8]) -> Result<SelfRef<&[u8]>, TelexError<E>>;
// }
//
...
// receive a TelexError::Cancelled error. This is to ensure that the caller
...
/// automatically receive a [`TelexError::Cancelled`] error. This guarantees
...
/// will automatically receive a [`crate::TelexError::Cancelled`] error.
pub trait ReplySink: MaybeSend + MaybeSync + 'static {
        ...
        Self: Sized,
    {
        ...
        // Wire format is always Result<T, TelexError<E>>. We don't know T here,
        // but postcard encodes () as zero bytes, so Result<(), TelexError<E>>
        // produces the same Err variant encoding as any Result<T, TelexError<E>>.
        async move {
            let wire: Result<(), TelexError<E>> = Err(error);
    ...
    /// Send an error response using the full wire shape `Result<T, TelexError<E>>`.
    ///
        ...
        E: facet::Facet<'wire> + MaybeSend,
    {
        use crate::{Payload, RequestResponse};
        async move {
            let wire: Result<T, TelexError<E>> = Err(error);
            let ptr = facet::PtrConst::new((&wire as *const Result<T, TelexError<E>>).cast::<u8>());
            let shape = <Result<T, TelexError<E>> as facet::Facet<'wire>>::SHAPE;
...
pub trait Handler<R: ReplySink>: MaybeSend + MaybeSync + 'static {
    ...
    ///
    /// This is the full wire type `Result<T, TelexError<E>>`, not the
    ...
    R: ReplySink,
{
    async fn reply(self, result: Result<T, E>) {
        use crate::{Payload, RequestResponse};
        let wire: Result<T, crate::TelexError<E>> = result.map_err(crate::TelexError::User);
        let ptr =
            facet::PtrConst::new((&wire as *const Result<T, crate::TelexError<E>>).cast::<u8>());
        let shape = <Result<T, crate::TelexError<E>> as facet::Facet<'wire>>::SHAPE;
...
mod tests {
    ...
    async fn reply_sink_send_error_uses_outgoing_payload_and_reply_path() {
        ...
        sink.send_error(crate::TelexError::<String>::Cancelled).await;
    ...
    async fn reply_sink_send_typed_error_preserves_ok_shape() {
        use crate::{
            TelexError, SchemaKind, TypeRef, VariantPayload, build_registry, extract_schemas,
        };
        ...
        .send_typed_error::<(String, i32), String>(TelexError::Cancelled)
        .await;
        ...
        let extracted =
            extract_schemas(<Result<(String, i32), TelexError<String>> as facet::Facet>::SHAPE)
