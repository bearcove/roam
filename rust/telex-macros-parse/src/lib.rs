...
//! Parser grammar for telex RPC service trait definitions.
//!
//! # This Is Just a Grammar
//!
//! This crate contains **only** the [unsynn] grammar for parsing Rust trait definitions
//! that define telex RPC services. It does not:
//!
//! - Generate any code
//! - Perform validation
//! - Know anything about telex's wire protocol
...
//!
//! 3. **Separation of concerns** — The grammar is pure parsing; [`telex-macros`] handles
//!    the proc-macro machinery; [`telex-codegen`] handles actual code generation.
//!
//! # The Bigger Picture
//!
//! ```text
//! telex-macros-parse     telex-macros              telex-codegen
//! ┌──────────────┐     ┌──────────────┐         ┌──────────────┐
//! │              │     │              │         │              │
...
//! [`telex-macros`]: https://docs.rs/telex-service-macros
//! [`telex-codegen`]: https://docs.rs/telex-codegen
...
impl Type {
    ...
    ///
    /// We require explicit `'telex` for borrowed RPC return payloads.
    pub fn has_elided_reference_lifetime(&self) -> bool {
        match self {
...
mod tests {
    ...
    fn type_helpers_detect_named_and_elided_lifetimes() {
        let trait_def = parse(
            r#"
            trait Svc {
                async fn borrowed(&self) -> Result<&'telex str, Error>;
        ...
        assert!(borrowed_ok.has_named_lifetime("telex"));
        assert!(!borrowed_ok.has_non_named_lifetime("telex"));
        ...
        assert!(!bad_ok.has_named_lifetime("telex"));
        assert!(bad_ok.has_non_named_lifetime("telex"));
        ...
        assert!(!elided_ok.has_named_lifetime("telex"));
        assert!(!elided_ok.has_non_named_lifetime("telex"));
