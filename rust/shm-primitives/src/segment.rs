...
/// Magic bytes that identify a telex SHM segment.
///
...
/// The segment header lives at offset 0 of every telex SHM segment.
///
...
pub struct SegmentHeader {
    /// "TELXHUB\x07" — identifies a telex SHM segment.
    pub magic: [u8; 8],
...
impl SegmentHeader {
    ...
    /// Validate that the header looks like a telex segment.
    ///
    ...
    pub fn validate(&self) -> Result<(), &'static str> {
        if self.magic != MAGIC {
            return Err("bad magic: not a telex segment");
        }
