use facet::Facet;
use telex_postcard::error::DeserializeError;
use telex_postcard::plan::{PlanInput, SchemaSet, TranslationPlan, build_plan};
use telex_types::{BindingDirection, MethodId, Schema, SchemaRecvTracker, TypeRef, extract_schemas};
    ...
    'input: 'facet,
{
    ...
    telex_postcard::from_slice_borrowed_with_plan(bytes, &resolved.plan, &resolved.remote.registry)
}
...
pub fn schema_deserialize_response<T: Facet<'static>>(
    ...
    let resolved = resolve_plan::<T>(method_id, BindingDirection::Response, tracker)?;
    telex_postcard::from_slice_with_plan(bytes, &resolved.plan, &resolved.remote.registry)
}
...
mod tests {
    ...
    fn schema_deserialize_args_handles_nested_unary_tuple() {
        ...
        let bytes =
            telex_postcard::to_vec(&((42i32, "hello".to_string()),)).expect("serialize tuple args");
