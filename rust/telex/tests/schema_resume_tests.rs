...
use telex_types::{ConnectionSettings, Parity};
    ...
    let (resume_result, server_accept_result) = tokio::join!(
        async {
            let mut resumed_link = initiate_transport(client_link2, TransportMode::Bare)
                .await
                .expect("transport");
            let handshake_result = telex_core::handshake_as_initiator(
                &resumed_link.tx,
                &mut resumed_link.rx,
                ConnectionSettings {
                    parity: Parity::Odd,
            ...
            .expect("handshake");
            let message_plan =
                telex_core::MessagePlan::from_handshake(&handshake_result).expect("message plan");
