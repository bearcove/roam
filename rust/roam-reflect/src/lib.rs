...
use roam_schema::{FieldDetail, TypeDetail, VariantDetail, VariantPayload};
...
impl Ctx {
    ...
    fn type_detail_from_shape_uncached(
        ...
        // roam-specific wire mapping for Push<T> (caller→callee stream).
        if has_attr(shape.attributes, Some("roam"), "push") {
            let Some(tp) = shape.type_params.first() else {
                return Err(Error::new(
                    "roam::push marker requires a single type parameter",
                ));
            };
...
mod tests {
    ...
    fn stream_detection_works() {
        ...
        assert!(contains_stream(&TypeDetail::Tx(Box::new(
            TypeDetail::String
        ))));
    ...
    #[test]
    fn nested_stream_generates_nested_type() {
        use roam_session::Tx;
        let td = type_detail::<Tx<Vec<u8>>>().unwrap();
        assert_eq!(td, TypeDetail::Tx(Box::new(TypeDetail::Bytes)));
    }
}
