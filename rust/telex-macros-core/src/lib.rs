//! Code generation core for telex RPC service macros.
//!
...
/// Returns the token stream for accessing the `telex` crate.
pub fn telex_crate() -> TokenStream2 {
    match crate_name::crate_name("telex") {
...
///
/// Takes a `telex` token stream (the path to the telex crate) so that this function
/// can be called from tests with a fixed path like `::telex`.
pub fn generate_service(parsed: &ServiceTrait, telex: &TokenStream2) -> Result<TokenStream2, Error> {
    ...
    for method in parsed.methods() {
        ...
        if method.is_persist() && method.args().any(|arg| arg.ty.contains_channel()) {
            return Err(Error::new(
                proc_macro2::Span::call_site(),
                format!(
                    "method `{}` declares `#[telex(persist)]` but has Channel (Tx/Rx) arguments - persist methods cannot carry channels",
                    method.name()
                ),
            ));
        }
        ...
        if ok_ty.has_elided_reference_lifetime() {
            return Err(Error::new(
                proc_macro2::Span::call_site(),
                format!(
                    "method `{}` return type uses an elided reference lifetime; use explicit `'telex` (for example `&'telex str`)",
                    method.name()
                ),
            ));
        }
        if ok_ty.has_non_named_lifetime("telex") {
            return Err(Error::new(
                proc_macro2::Span::call_site(),
                format!(
                    "method `{}` return type may only use lifetime `'telex` for borrowed response data",
                    method.name()
                ),
            ));
        }
    ...
    let service_descriptor_fn = generate_service_descriptor_fn(parsed, telex);
    let service_trait = generate_service_trait(parsed, telex);
    let dispatcher = generate_dispatcher(parsed, telex);
    let client = generate_client(parsed, telex);
    Ok(quote! {
...
fn generate_service_descriptor_fn(parsed: &ServiceTrait, telex: &TokenStream2) -> TokenStream2 {
    ...
    let method_descriptors: Vec<TokenStream2> = parsed
        ...
        .map(|m| {
            ...
            quote! {
                #telex::hash::method_descriptor_with_retry::<#args_tuple_ty, #return_ty_tokens>(
                    #service_name,
                    #method_name_str,
                    &[#(#arg_name_strs),*],
                    #method_doc_expr,
    ...
    quote! {
        ...
        pub fn #descriptor_fn_name() -> &'static #telex::session::ServiceDescriptor {
            static DESCRIPTOR: std::sync::OnceLock<&'static #telex::session::ServiceDescriptor> = std::sync::OnceLock::new();
            DESCRIPTOR.get_or_init(|| {
                let methods: Vec<&'static #telex::session::MethodDescriptor> = vec![
                    #(#method_descriptors),*
                ];
...
fn generate_service_trait(parsed: &ServiceTrait, telex: &TokenStream2) -> TokenStream2 {
    let trait_name = parsed.name.clone();
    let trait_doc = parsed.doc().map(|d| quote! { #[doc = #d] });

    let methods: Vec<TokenStream2> = parsed
        .methods()
        .map(|m| generate_trait_method(m, telex))
        .collect();
...
fn generate_trait_method(method: &ServiceMethod, telex: &TokenStream2) -> TokenStream2 {
    ...
    let ok_has_telex_lifetime = ok_ty_ref.has_named_lifetime("telex");
    let method_lifetime = if ok_has_telex_lifetime {
        quote! { <'telex> }
    } else {
        quote! {}
    };
    ...
    let context_param = wants_context.then(|| quote! { cx: &#telex::RequestContext<'_> });

    if ok_has_telex_lifetime {
        ...
        signature_params.push(quote! { call: impl #telex::Call<'telex, #ok_ty, #err_ty> });
        signature_params.extend(params);
        quote! {
            #method_doc
            fn #method_name #method_lifetime (&self, #(#signature_params),*) -> impl std::future::Future<Output = ()> + #telex::MaybeSend;
        }
    } else {
        ...
        quote! {
            #method_doc
            fn #method_name (&self, #(#signature_params),*) -> impl std::future::Future<Output = #output_ty> + #telex::MaybeSend;
        }
    }
}
...
fn generate_dispatcher(parsed: &ServiceTrait, telex: &TokenStream2) -> TokenStream2 {
    let trait_name = parsed.name.clone();
    ...
    let dispatch_arms: Vec<TokenStream2> = parsed
        .methods()
        .enumerate()
        .map(|(i, m)| generate_dispatch_arm(m, i, telex, &descriptor_fn_name))
        .collect();
    ...
    let response_wire_shape_arms: Vec<TokenStream2> = parsed
        ...
        .map(|(i, m)| {
            ...
            quote! {
                if method_id == #descriptor_fn_name().methods[#i].id {
                    return Some(
                        <Result<#ok_ty, #telex::TelexError<#err_ty>> as #telex::facet::Facet<'static>>::SHAPE,
                    );
                }
            }
        })
    ...
    let dispatch_body = if no_methods {
        ...
        quote! {
            ...
            let args_bytes = match &call.args {
                ...
                _ => {
                    reply.send_error(#telex::TelexError::<::core::convert::Infallible>::InvalidPayload("args not PostcardBytes".into())).await;
                    return;
                }
            };
            #(#dispatch_arms)*
            reply.send_error(#telex::TelexError::<::core::convert::Infallible>::UnknownMethod).await;
        }
    };
    ...
    quote! {
        ...
        ///
        /// Wraps a handler and implements [`#telex::Handler`] by routing incoming
        ...
        where
            H: #trait_name + Clone + #telex::MaybeSend + #telex::MaybeSync + 'static,
        {
        ...
        impl<H, R> #telex::Handler<R> for #dispatcher_name<H>
        where
            H: #trait_name + Clone + #telex::MaybeSend + #telex::MaybeSync + 'static,
            R: #telex::ReplySink,
        {
            ...
            async fn handle(&self, call: #telex::SelfRef<#telex::RequestCall<'static>>, reply: R, schemas: ::std::sync::Arc<#telex::SchemaRecvTracker>) {
                #dispatch_body
            }
        }
    }
...
fn generate_dispatch_arm(
    ...
    // Replace 'telex with 'static so the Shape reference is valid in the dispatch scope.
    let ok_ty_dispatch: proc_macro2::TokenStream = ok_ty
        .to_string()
        .replace("'telex", "'static")
        .parse()
    ...
    let invoke_and_reply = if ok_has_telex_lifetime {
        quote! {
        ...
        quote! {
            ...
            let sink_call = #telex::SinkCall::new(reply);
            #telex::Call::<'_, #ok_ty, #err_ty>::reply(sink_call, result).await;
            if !self.middlewares.is_empty() {
                self.run_post_hooks(&context, outcome_handle.outcome()).await;
            }
        }
        ...
        quote! {
            ...
            let sink_call = #telex::SinkCall::new(reply);
            #telex::Call::<'_, #ok_ty, #err_ty>::ok(sink_call, value).await;
            if !self.middlewares.is_empty() {
                self.run_post_hooks(&context, outcome_handle.outcome()).await;
            }
        }
        ...
        if method_id == #descriptor_fn_name().methods[#idx].id {
            ...
            let deser_result = if let Some(binder) = reply.channel_binder() {
                #telex::with_channel_binder(binder, || {
                    #telex::schema_deser::schema_deserialize_args_borrowed::<#args_tuple_type>(
                        args_bytes,
                        method_id,
                        &schemas,
                    )
...
// r[impl rpc.caller]
fn generate_client(parsed: &ServiceTrait, telex: &TokenStream2) -> TokenStream2 {
    ...
    let client_methods: Vec<TokenStream2> = parsed
        .methods()
        .enumerate()
        .map(|(i, m)| generate_client_method(m, i, &descriptor_fn_name, telex))
        .collect();
    ...
    quote! {
        ...
        impl From<#telex::DriverCaller> for #client_name {
...
fn generate_client_method(
    ...
    let channel_retry_mode = if method.args().any(|arg| arg.ty.contains_channel()) {
        if method.is_idem() {
            quote! { #telex::ChannelRetryMode::Idem }
        } else {
            quote! { #telex::ChannelRetryMode::NonIdem }
        }
    } else {
        quote! { #telex::ChannelRetryMode::None }
    };
    ...
    let ok_uses_telex_lifetime = ok_type_for_lifetimes.has_named_lifetime("telex");
    let (ok_ty_decode, err_ty, client_return) = if let Some((ok, err)) = return_type.as_result() {
        ...
        (
            ...
            err_t.clone(),
            if ok_uses_telex_lifetime {
                quote! { Result<#telex::SelfRef<#ok_t_static>, #telex::TelexError<#err_t>> }
            } else {
                quote! { Result<#ok_t, #telex::TelexError<#err_t>> }
            },
        )
    } else {
        ...
        (
            ...
            if ok_uses_telex_lifetime {
                quote! { Result<#telex::SelfRef<#t_static>, #telex::TelexError> }
            } else {
                quote! { Result<#t, #telex::TelexError> }
            },
        )
    };
    ...
    if ok_uses_telex_lifetime {
        quote! {
            ...
            pub async fn #method_name(&self, #(#params),*) -> #client_return {
                ...
                #telex::ensure_channel_retry_mode(&mut metadata, #channel_retry_mode);
                ...
                let with_tracker = match #telex::Caller::call(&self.caller, req).await {
                    ...
                    Err(e) => {
                        ...
                        return Err(match e {
                            #telex::TelexError::UnknownMethod => #telex::TelexError::<#err_ty>::UnknownMethod,
                            #telex::TelexError::InvalidPayload(msg) => #telex::TelexError::<#err_ty>::InvalidPayload(msg),
                            #telex::TelexError::Cancelled => #telex::TelexError::<#err_ty>::Cancelled,
                            #telex::TelexError::ConnectionClosed => #telex::TelexError::<#err_ty>::ConnectionClosed,
                            #telex::TelexError::SessionShutdown => #telex::TelexError::<#err_ty>::SessionShutdown,
                            #telex::TelexError::SendFailed => #telex::TelexError::<#err_ty>::SendFailed,
                            #telex::TelexError::Indeterminate => #telex::TelexError::<#err_ty>::Indeterminate,
                            #telex::TelexError::User(never) => match never {},
                        });
                    }
                };
                let #telex::WithTracker { value: response, tracker: schema_tracker } = with_tracker;
                response.try_repack(|resp, _bytes| {
                    let ret_bytes = match &resp.ret {
                        #telex::Payload::PostcardBytes(bytes) => bytes,
                        _ => return Err(#telex::TelexError::<#err_ty>::InvalidPayload("response not PostcardBytes".into())),
                    };
                    let result: Result<#ok_ty_decode, #telex::TelexError<#err_ty>> =
                        #telex::schema_deser::schema_deserialize_response_borrowed::<Result<#ok_ty_decode, #telex::TelexError<#err_ty>>>(ret_bytes, method_id, &schema_tracker)
                            .map_err(|e| {
                                #finish_retry_bindings
                                #telex::TelexError::<#err_ty>::InvalidPayload(e.to_string())
                            })?;
                    match result {
                        Ok(ret) => Ok(ret),
                        Err(err) => {
        ...
        quote! {
            ...
            pub async fn #method_name(&self, #(#params),*) -> #client_return {
                ...
                let result: Result<#ok_ty_decode, #telex::TelexError<#err_ty>> =
                    #telex::schema_deser::schema_deserialize_response::<Result<#ok_ty_decode, #telex::TelexError<#err_ty>>>(
                        ret_bytes,
                        method_id,
                        &schema_tracker,
                    )
...
mod tests {
    ...
    fn generate(input: proc_macro2::TokenStream) -> String {
        let parsed = telex_macros_parse::parse_trait(&input).unwrap();
        let telex = quote! { ::telex };
        let ts = crate::generate_service(&parsed, &telex).unwrap();
        prettyprint(ts)
    }
    ...
    fn explicit_request_context_opt_in() {
        assert_snapshot!(generate(quote! {
            trait Audit {
                #[telex::context]
                async fn record(&self, payload: String) -> &'telex str;
    ...
    fn rejects_channels_in_return_type() {
        let parsed = telex_macros_parse::parse_trait(&quote! {
            trait Streamer { async fn stream(&self) -> Rx<i32>; }
        })
        .unwrap();
        let telex = quote! { ::telex };
        let err = crate::generate_service(&parsed, &telex).unwrap_err();
        assert_eq!(
            err.message,
    ...
    fn rejects_persist_methods_with_channel_arguments() {
        ...
        assert_eq!(
            err.message,
            "method `stream` declares `#[telex(persist)]` but has Channel (Tx/Rx) arguments - persist methods cannot carry channels"
        );
    }

    #[test]
    fn rejects_non_telex_return_lifetime() {
        let parsed = telex_macros_parse::parse_trait(&quote! {
        ...
        assert_eq!(
            err.message,
            "method `bad` return type may only use lifetime `'telex` for borrowed response data"
        );
    }
    ...
    fn rejects_elided_return_lifetime() {
        ...
        assert_eq!(
            err.message,
            "method `bad` return type uses an elided reference lifetime; use explicit `'telex` (for example `&'telex str`)"
        );
    }
    ...
    fn rejects_borrowed_error_type() {
        let parsed = telex_macros_parse::parse_trait(&quote! {
            trait Svc { async fn bad(&self) -> Result<u32, &'telex str>; }
        })
        .unwrap();
        let telex = quote! { ::telex };
        let err = crate::generate_service(&parsed, &telex).unwrap_err();
    ...
    #[test]
    fn borrowed_telex_return() {
        assert_snapshot!(generate(quote! {
            trait Hasher { async fn hash(&self, payload: String) -> &'telex str; }
        }));
    }

    #[test]
    fn borrowed_telex_return_call_style() {
        assert_snapshot!(generate(quote! {
            trait Hasher { async fn hash(&self, payload: String) -> &'telex str; }
        }));
    }
    ...
    #[test]
    fn borrowed_telex_cow_return() {
        assert_snapshot!(generate(quote! {
            trait TextSvc {
                async fn normalize(&self, input: String) -> ::std::borrow::Cow<'telex, str>;
            }
        }));
    }
    ...
    fn borrowed_return_mixed_with_borrowed_args_and_channels_compiles_to_expected_shapes() {
        assert_snapshot!(generate(quote! {
            trait WordLab {
                ...
                async fn classify(&self, word: String) -> &'telex str;
