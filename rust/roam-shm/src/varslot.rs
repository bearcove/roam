    ...
    pub unsafe fn init(region: Region, base_offset: usize, configs: &[SizeClassConfig]) -> Self {
        ...
        for (i, (cfg, offsets)) in configs.iter().zip(layout.class_offsets.iter()).enumerate() {
            ...
            for slot in 0..cfg.slot_count {
                ...
                m.generation = shm_primitives::sync::AtomicU32::new(0);
                m.state = shm_primitives::sync::AtomicU32::new(SlotState::Free as u32);
                m.owner_peer = shm_primitives::sync::AtomicU32::new(0);
                m.next_free = shm_primitives::sync::AtomicU32::new(if slot + 1 < cfg.slot_count {
                    slot + 1
                } else {
                    EMPTY
                });
