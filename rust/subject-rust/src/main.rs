//! Rust subject binary for the telex compliance suite.
...
use telex::{Rx, Tx};
use telex_core::{TransportMode, initiator, initiator_on};
use telex_shm::bootstrap::{BootstrapStatus, encode_request};
use telex_shm::segment::Segment;
use telex_stream::tcp_connector;
...
async fn listen_and_serve() -> Result<(), String> {
    ...
    let (_client, _sh) = acceptor_on(telex_stream::StreamLink::tcp(stream))
    ...
    match scenario.as_str() {
        ...
        "divide_zero" => {
            match client.divide(10, 0).await {
                Err(telex::TelexError::User(MathError::DivisionByZero)) => {}
                other => {
                    return Err(format!(
        ...
        "divide_overflow" => {
            match client.divide(i64::MIN, -1).await {
                Err(telex::TelexError::User(MathError::Overflow)) => {}
        ...
        "lookup_not_found" => {
            match client.lookup(999).await {
                Err(telex::TelexError::User(spec_proto::LookupError::NotFound)) => {}
                other => {
                    return Err(format!(
        ...
        "lookup_access_denied" => {
            match client.lookup(100).await {
                Err(telex::TelexError::User(spec_proto::LookupError::AccessDenied)) => {}
                other => {
                    return Err(format!(
        ...
        "transform_bidi" => {
            let (input_tx, input_rx) = telex::channel::<String>();
            let (output_tx, mut output_rx) = telex::channel::<String>();
        ...
        "channel_retry_non_idem" => {
            ...
            if !matches!(result, Err(telex::TelexError::Indeterminate)) {
                return Err(format!(
...
async fn connect_and_serve_shm() -> Result<(), String> {
    ...
    let link = {
        ...
        let mut stream = telex_local::connect(&control_sock)
            .await
