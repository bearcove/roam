...
// ============================================================================
// telex zerocopy-vs-owned
// ============================================================================

mod telex_zerocopy_bench {
    use moire::task::FutureExt;
    use telex_core::{NoopCaller, TransportMode, acceptor_on, initiator_on, memory_link_pair};
    use telex_shm::varslot::SizeClassConfig;
    use telex_shm::{Segment, SegmentConfig, create_test_link_pair};
...
define_zerocopy_transport_benches!(
    telex_mem_zerocopy_owned_len,
    telex_mem_zerocopy_borrowed_len,
    telex_zerocopy_bench::setup_mem,
    "telex-mem"
);
define_zerocopy_transport_benches!(
    telex_tcp_zerocopy_owned_len,
    telex_tcp_zerocopy_borrowed_len,
    telex_zerocopy_bench::setup_tcp,
    "telex-tcp"
);
define_zerocopy_transport_benches!(
    telex_shm_zerocopy_owned_len,
    telex_shm_zerocopy_borrowed_len,
    telex_zerocopy_bench::setup_shm,
    "telex-shm"
);
...
mod telex_bench {
    ...
    pub trait Bench {
        ...
        async fn generate(&self, count: u32, output: telex::Tx<i32>);
    }
...
#[divan::bench]
fn telex_add(bencher: divan::Bencher) {
    let client = TOKIO.block_on(telex_bench::setup());
    bencher.bench_local(|| {
        TOKIO.block_on(async {
            let resp = client.add(3, 5).await.expect("telex add failed");
            divan::black_box(resp);
        })
    });
}
...
fn telex_echo(bencher: divan::Bencher, n: usize) {
    let client = TOKIO.block_on(telex_bench::setup());
...
fn telex_stream(bencher: divan::Bencher, n: usize) {
    let client = TOKIO.block_on(telex_bench::setup());
    bencher.bench_local(|| {
        TOKIO.block_on(async {
            let (tx, mut rx) = telex::channel::<i32>();
            ...
            call.await.expect("telex generate failed");
...
// ============================================================================
// telex-shm (shared memory transport)
// ============================================================================
...
mod telex_shm_bench {
    ...
    use moire::task::FutureExt;
    use telex_core::{NoopCaller, TransportMode, acceptor_on, initiator_on};
    use telex_shm::varslot::SizeClassConfig;
    use telex_shm::{Segment, SegmentConfig, create_test_link_pair};

    use shm_primitives::FileCleanup;

    use super::telex_bench::{BenchClient, BenchDispatcher, Handler};
...
#[divan::bench]
fn telex_shm_add(bencher: divan::Bencher) {
    let client = TOKIO.block_on(telex_shm_bench::setup());
    bencher.bench_local(|| {
        TOKIO.block_on(async {
            let resp = client.add(3, 5).await.expect("telex-shm add failed");
            divan::black_box(resp);
        })
    });
}
...
#[divan::bench(args = PAYLOAD_SIZES)]
fn telex_shm_echo(bencher: divan::Bencher, n: usize) {
    let client = TOKIO.block_on(telex_shm_bench::setup());
    let payload = vec![42u8; n];
    bencher.bench_local(|| {
        TOKIO.block_on(async {
...
#[divan::bench(args = STREAM_COUNTS)]
fn telex_shm_stream(bencher: divan::Bencher, n: usize) {
    let client = TOKIO.block_on(telex_shm_bench::setup());
    bencher.bench_local(|| {
        TOKIO.block_on(async {
            let (tx, mut rx) = telex::channel::<i32>();
            ...
            call.await.expect("telex-shm generate failed");
            let count = recv_task.await.unwrap();
            divan::black_box(count);
        })
    });
...
// ============================================================================
// telex-tcp (TCP loopback transport)
// ============================================================================
...
#[divan::bench]
fn telex_tcp_add(bencher: divan::Bencher) {
    let client = TOKIO.block_on(telex_tcp_bench::setup());
    bencher.bench_local(|| {
        TOKIO.block_on(async {
            let resp = client.add(3, 5).await.expect("telex-tcp add failed");
            divan::black_box(resp);
        })
    });
}
...
#[divan::bench(args = PAYLOAD_SIZES)]
fn telex_tcp_echo(bencher: divan::Bencher, n: usize) {
    let client = TOKIO.block_on(telex_tcp_bench::setup());
    let payload = vec![42u8; n];
    bencher.bench_local(|| {
        TOKIO.block_on(async {
...
#[divan::bench(args = STREAM_COUNTS)]
fn telex_tcp_stream(bencher: divan::Bencher, n: usize) {
    let client = TOKIO.block_on(telex_tcp_bench::setup());
    bencher.bench_local(|| {
        TOKIO.block_on(async {
            let (tx, mut rx) = telex::channel::<i32>();
            ...
            call.await.expect("telex-tcp generate failed");
            let count = recv_task.await.unwrap();
            divan::black_box(count);
        })
    });
...
mod serialize_bench {
    ...
    pub fn make_message<'a>(args: &'a (&'a UserProfile<'a>,)) -> Message<'a> {
        Message {
            connection_id: telex_types::ConnectionId(1),
            payload: MessagePayload::RequestMessage(RequestMessage {
                id: telex_types::RequestId(42),
                body: RequestBody::Call(RequestCall {
                    method_id: telex_types::MethodId(7),
    ...
    pub fn make_blob_msg<'a>(args: &'a (&'a BlobMessage<'a>,)) -> Message<'a> {
        Message {
            ...
            payload: MessagePayload::RequestMessage(RequestMessage {
                ...
                body: RequestBody::Call(RequestCall {
                    method_id: telex_types::MethodId(1),
                    args: Payload::outgoing(args),
                    metadata: vec![],
                    schemas: Default::default(),
                }),
...
fn message_postcard_plan_decode(bencher: divan::Bencher, n: usize) {
    let fixture = serialize_bench::make_exact_call_fixture(n);
    let msg = fixture.message();
    let bytes = telex_postcard::to_vec(&msg).expect("serialize");
