...
//! the sentinel shape defined in telex-schema (`RawPostcardBorrowed`), we write the
//! bytes directly (passthrough) instead of recursively serializing.

use facet_core::PtrConst;

pub use telex_schema::{RAW_POSTCARD_BORROWED_SHAPE, RawPostcardBorrowed};
...
pub fn opaque_encoded_borrowed(bytes: &&[u8]) -> facet::OpaqueSerialize {
    telex_schema::opaque_encoded_borrowed(bytes)
}
...
///
/// Checks against the sentinel shape defined in telex-schema using value equality.
///
/// # Safety
