...
mod tests {
    ...
    fn plan_for(
        ...
        let remote_extracted =
            telex_types::extract_schemas(remote_shape).expect("schema extraction");
        let remote =
            SchemaSet::from_root_and_schemas(remote_extracted.root, remote_extracted.schemas);
        let local_extracted = telex_types::extract_schemas(local_shape).expect("schema extraction");
    ...
    fn translation_bytes_and_list_u8_are_compatible() {
        let remote = SchemaSet::from_schemas(vec![
            telex_types::Schema {
                id: telex_types::SchemaHash(1),
                type_params: vec![],
                kind: telex_types::SchemaKind::Primitive {
                    primitive_type: telex_types::PrimitiveType::U8,
                },
            },
            telex_types::Schema {
                id: telex_types::SchemaHash(2),
                type_params: vec![],
                kind: telex_types::SchemaKind::List {
                    element: telex_types::TypeRef::concrete(telex_types::SchemaHash(1)),
                },
            },
        ]);
        let local_extracted =
            telex_types::extract_schemas(<Vec<u8> as Facet>::SHAPE).expect("schema extraction");
        let local = SchemaSet::from_root_and_schemas(local_extracted.root, local_extracted.schemas);
        let plan = build_plan(&PlanInput {
            remote: &remote,
            local: &local,
    ...
    #[test]
    fn translation_result_with_telex_error_shape() {
        use telex_types::TelexError;
        use std::convert::Infallible;

        type T = Result<String, TelexError<Infallible>>;
        let r = plan_for(T::SHAPE, T::SHAPE).unwrap();
        let bytes = to_vec(&Ok::<_, TelexError<Infallible>>("hello".to_string())).unwrap();
        let result: T = from_slice_with_plan(&bytes, &r.plan, &r.remote.registry).unwrap();
        assert_eq!(result.unwrap(), "hello");
    }

    #[test]
    fn translation_result_with_telex_error_err_variant() {
        use telex_types::TelexError;
        use std::convert::Infallible;

        type T = Result<String, TelexError<Infallible>>;
        let r = plan_for(T::SHAPE, T::SHAPE).unwrap();
        let bytes = to_vec(&Err::<String, _>(TelexError::<Infallible>::InvalidPayload(
            "bad data".into(),
        )))
        .unwrap();
        let result: T = from_slice_with_plan(&bytes, &r.plan, &r.remote.registry).unwrap();
        match result {
            Err(TelexError::InvalidPayload(msg)) => assert_eq!(msg, "bad data"),
    ...
    fn opaque_round_trip() {
        ...
        eprintln!("outgoing - telex:  {:02x?}", our_bytes);
        ...
        eprintln!("incoming - telex:  {:02x?}", reserialized);
        assert_eq!(
    ...
    fn schema_set_from_schemas_works() {
        let schemas = telex_types::extract_schemas(<Vec<u32> as Facet>::SHAPE)
            .expect("schema extraction")
            .schemas;
        let set = SchemaSet::from_schemas(schemas);
        assert!(
            matches!(set.root.kind, telex_types::SchemaKind::List { .. }),
            "root should be List"
        );
    }
    ...
    fn translation_error_display_coverage() {
        ...
        let dummy_prim = Schema {
            ...
            kind: SchemaKind::Primitive {
                primitive_type: telex_types::PrimitiveType::U32,
            },
        };
