...
use telex_schema::{PrimitiveType, SchemaKind, SchemaRegistry};
    ...
    match kind {
        ...
        SchemaKind::Enum { variants, .. } => {
            ...
            match &variant.payload {
                telex_schema::VariantPayload::Unit => Ok(()),
                telex_schema::VariantPayload::Newtype { type_ref } => {
                    let inner_kind = resolve_kind(type_ref, registry)?;
                    skip_value(cursor, &inner_kind, registry)
                }
                telex_schema::VariantPayload::Tuple { types } => {
                ...
                telex_schema::VariantPayload::Struct { fields } => {
                    for field in fields {
                        let field_kind = resolve_kind(&field.type_ref, registry)?;
                        skip_value(cursor, &field_kind, registry)?;
                    }
...
fn resolve_kind(
    type_ref: &telex_schema::TypeRef,
    registry: &SchemaRegistry,
) -> Result<telex_schema::SchemaKind, DeserializeError> {
