...
/// A [`Link`](telex_types::Link) over a WebSocket connection.
///
/// Wraps a [`WebSocketStream`] and sends each telex payload as a single
            ...
            match self.rx.recv().await {
                ...
                Some(Ok(WsMessage::Text(_))) => {
                    return Err(WsLinkRxError(io::Error::new(
                        ...
                        "text frames not allowed on telex websocket link",
                    )));
                }
                Some(Err(e)) => {
