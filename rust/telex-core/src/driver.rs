...
use moire::sync::mpsc;
use telex_types::{OperationId, PostcardPayload, SchemaHash, TypeRef};
...
mod tests {
    ...
    use telex_types::{ChannelCreditReplenisher, ChannelId};
...
/// `TelexError::Cancelled` to the caller. This guarantees that every
...
async fn replay_sealed_response(
    ...
    let mut response: RequestResponse<'_> =
        telex_postcard::from_slice_borrowed(encoded_response).map_err(|_| ())?;
...
fn extract_root_type_ref(schemas_cbor: &telex_types::CborPayload) -> TypeRef {
    if schemas_cbor.is_empty() {
        return TypeRef::concrete(SchemaHash(0));
    }
    let payload =
        telex_types::SchemaPayload::from_cbor(&schemas_cbor.0).expect("schema CBOR must be valid");
    payload.root
}
...
impl ReplySink for DriverReplySink {
    async fn send_reply(mut self, response: RequestResponse<'_>) {
        ...
        if let Payload::Value { shape, .. } = &response.ret
            && let Ok(extracted) = telex_types::extract_schemas(shape)
        {
            telex_types::dlog!(
        ...
        if let (Some(operation_id), Some(operations)) = (self.operation_id, self.operations.take())
        {
            ...
            let encoded_for_store = PostcardPayload(
                telex_postcard::to_vec(&response).expect("serialize operation response for store"),
            );
...
impl ChannelSink for DriverChannelSink {
    ...
    ) -> Pin<Box<dyn telex_types::MaybeSendFuture<Output = Result<(), TxError>> + 'payload>> {
    ...
    fn close_channel(
        &self,
        _metadata: telex_types::Metadata,
    ) -> Pin<Box<dyn telex_types::MaybeSendFuture<Output = Result<(), TxError>> + 'static>> {
...
fn register_rx_channel_impl(
    ...
    local_control_tx: mpsc::UnboundedSender<DriverLocalControl>,
) -> telex_types::BoundChannelReceiver {
    ...
    if terminal_buffered {
        ...
        return telex_types::BoundChannelReceiver {
            receiver: rx,
            liveness,
            replenisher: None,
        };
...
impl DriverChannelBinder {
    ...
    fn register_rx_channel(&self, channel_id: ChannelId) -> telex_types::BoundChannelReceiver {
...
impl ChannelBinder for DriverChannelBinder {
    ...
    fn create_rx(&self) -> (ChannelId, telex_types::BoundChannelReceiver) {
        let channel_id = self.shared.channel_ids.lock().alloc();
    ...
    fn register_rx(&self, channel_id: ChannelId) -> telex_types::BoundChannelReceiver {
...
impl DriverCaller {
    ...
    pub fn register_rx_channel(&self, channel_id: ChannelId) -> telex_types::BoundChannelReceiver {
        register_rx_channel_impl(
            &self.shared,
            channel_id,
...
impl Caller for DriverCaller {
    async fn call<'a>(&'a self, mut call: RequestCall<'a>) -> CallResult {
            ...
            .is_err()
        {
            ...
            return Err(TelexError::SendFailed);
        }
            ...
            tokio::select! {
                result = &mut response => {
                    match result {
                        ...
                        Err(_) => {
                            return Err(TelexError::ConnectionClosed);
                        }
                    }
                }
                changed = resumed_rx.changed(), if self.peer_supports_retry => {
                    telex_types::dlog!("[CALLER] resumed_rx fired");
                    if changed.is_err() {
                        self.shared.pending_responses.lock().remove(&req_id);
                        return Err(TelexError::SessionShutdown);
                    }
                    ...
                    match metadata_channel_retry_mode(&call.metadata) {
                        ChannelRetryMode::NonIdem => {
                            self.shared.pending_responses.lock().remove(&req_id);
                            return Err(TelexError::Indeterminate);
                        }
                ...
                changed = closed_rx.changed() => {
                    telex_types::dlog!("[CALLER] closed_rx fired, value={}", *closed_rx.borrow());
            ...
            tokio::select! {
                ...
                Some((req_id, disposition)) = self.failures_rx.recv() => {
                    ...
                    if !had_pending {
                        ...
                        let telex_error = match reply_disposition {
                            FailureDisposition::Cancelled => TelexError::Cancelled,
                            FailureDisposition::Indeterminate => TelexError::Indeterminate,
                        };
                        if let Some(method_id) = in_flight_method_id
                            && let Some(response_shape) = self.handler.response_wire_shape(method_id)
                            && let Ok(extracted) = telex_types::extract_schemas(response_shape)
                        {
                            let registry = telex_types::build_registry(&extracted.schemas);
                            let error: Result<(), TelexError<core::convert::Infallible>> =
                                Err(telex_error);
                            let encoded = telex_postcard::to_vec(&error)
        ...
        match control {
            ...
            DriverLocalControl::GrantCredit {
                ...
                let _ = self
                    ...
                    .send(ConnectionMessage::Channel(ChannelMessage {
                        id: channel_id,
                        body: ChannelBody::GrantCredit(telex_types::ChannelGrantCredit {
                            additional,
                        }),
                    }))
                    .await;
    ...
    fn handle_request(
        ...
        if is_call {
                ...
                match admit {
                    ...
                    AdmitResult::Conflict => {
                        ...
                        moire::task::spawn(
                            async move {
                                let error: Result<(), TelexError<core::convert::Infallible>> =
                                    Err(TelexError::InvalidPayload("operation ID conflict".into()));
                                let _ = sender
                                    .send_response(
                                        req_id,
                                        RequestResponse {
            ...
            if let Some(tx) = self.shared.pending_responses.lock().remove(&req_id) {
                telex_types::dlog!("[driver] routing response to waiter: req={:?}", req_id);
                tracing::debug!(%req_id, "routing response to pending oneshot");
                let _: Result<(), _> = tx.send(PendingResponse { msg, schemas });
            } else {
                telex_types::dlog!("[driver] dropped unmatched response: req={:?}", req_id);
            ...
            // mark_failure fires → failures_rx arm sends TelexError::Cancelled.
        }
    }
