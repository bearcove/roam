use telex_types::{Link, LinkRx, LinkTx, LinkTxPermit, SplitLink, TransportMode, WriteSlot};
use zerocopy::FromBytes;
use zerocopy::little_endian::U32 as LeU32;

const TRANSPORT_HELLO_MAGIC: u32 = u32::from_le_bytes(*b"TXTH");
const TRANSPORT_ACCEPT_MAGIC: u32 = u32::from_le_bytes(*b"TXTA");
const TRANSPORT_REJECT_MAGIC: u32 = u32::from_le_bytes(*b"TXTR");
...
async fn recv_bytes<LRx: LinkRx>(
    rx: &mut LRx,
) -> Result<telex_types::Backing, TransportPrologueError> {
    rx.recv()
        .await
        .map_err(|error| {
