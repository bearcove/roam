...
fn message_schema() -> Vec<Schema> {
    telex_types::extract_schemas(<telex_types::Message<'static> as facet::Facet<'static>>::SHAPE)
...
async fn recv_handshake<Rx: LinkRx>(rx: &mut Rx) -> Result<HandshakeMessage, HandshakeError> {
    ...
    telex_types::dlog!("[handshake] recv {:?}", handshake_tag(&msg));
    Ok(msg)
}
...
pub async fn handshake_as_initiator<Tx: LinkTx, Rx: LinkRx>(
    ...
    send_handshake(tx, &HandshakeMessage::LetsGo(telex_types::LetsGo {})).await?;
...
pub async fn handshake_as_acceptor<Tx: LinkTx, Rx: LinkRx>(
    ...
    if let Some(expected) = expected_resume_key {
        ...
        match actual {
            ...
            _ => {
                ...
                send_handshake(
                    tx,
                    &HandshakeMessage::Sorry(telex_types::Sorry {
                        reason: reason.clone(),
                    }),
                )
                .await?;
    ...
    let hy = telex_types::HelloYourself {
