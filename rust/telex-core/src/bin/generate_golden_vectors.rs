...
use telex_types::{
    ...
    RequestCall, RequestCancel, RequestId, RequestMessage, RequestResponse, TelexError,
};
...
fn encode_message(message: &Message<'_>) -> Vec<u8> {
    telex_postcard::to_vec(message).expect("serialize message fixture")
}
...
fn main() {
    macro_rules! write_value {
        ($path:literal, $value:expr) => {{
            let bytes = telex_postcard::to_vec(&$value).expect("serialize fixture");
    ...
    write_value!(
        ...
        Ok::<String, TelexError<std::convert::Infallible>>("hello".to_string())
    );
    write_value!(
        "result/ok_u32.bin",
        Ok::<u32, TelexError<std::convert::Infallible>>(42u32)
    );
    write_value!(
        "result/err_unknown_method.bin",
        Err::<(), TelexError<std::convert::Infallible>>(TelexError::UnknownMethod)
    );
    write_value!(
        "result/err_invalid_payload.bin",
        Err::<(), TelexError<std::convert::Infallible>>(TelexError::InvalidPayload(String::new()))
    );
    write_value!(
        "result/err_cancelled.bin",
        Err::<(), TelexError<std::convert::Infallible>>(TelexError::Cancelled)
    );
    write_value!(
        "result/err_user_string.bin",
        Err::<(), TelexError<String>>(TelexError::User("oops".to_string()))
    );
