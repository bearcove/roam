...
const CLIENT_HELLO_MAGIC: u32 = u32::from_le_bytes(*b"TXCH");
const SERVER_HELLO_MAGIC: u32 = u32::from_le_bytes(*b"TXSH");
...
impl<F: MsgFamily, LS: LinkSource> ConduitTxPermit for StableConduitPermit<F, LS> {
    ...
    fn send(self, item: F::Msg<'_>) -> Result<(), StableConduitError> {
        ...
        let plan = telex_postcard::peek_to_scatter_plan(peek).map_err(StableConduitError::Encode)?;
    ...
    LS: Send + 'static,
{
    ...
    async fn recv(&mut self) -> Result<Option<SelfRef<F::Msg<'static>>>, Self::Error> {
        loop {
            ...
            let item_backing = telex_types::Backing::Boxed(item_bytes.to_vec().into());
