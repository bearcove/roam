...
use telex_types::{Conduit, ConduitRx, ConduitTx, ConduitTxPermit, LinkRx, LinkTx, MsgFamily};
...
async fn send_frame<LTx: LinkTx>(tx: &LTx, seq: u32, ack: Option<u32>, item: &str) {
    let item_bytes = telex_postcard::to_vec(&item.to_string()).unwrap();
    let frame = Frame {
        seq: PacketSeq(seq),
    ...
    let plan = telex_postcard::peek_to_scatter_plan(peek).unwrap();
...
fn decode_frame(bytes: &[u8]) -> (u32, Option<u32>, String) {
    let frame: Frame<'_> = telex_postcard::from_slice_borrowed(bytes).unwrap();
    let item_bytes = match &frame.item {
        Payload::PostcardBytes(b) => b,
        _ => unreachable!("deserialized Payload should be Incoming"),
    };
    let item: String = telex_postcard::from_slice(item_bytes).unwrap();
...
async fn recv_raw<LRx: LinkRx>(rx: &mut LRx) -> Vec<u8> {
    ...
    match backing {
        telex_types::Backing::Boxed(b) => b.to_vec(),
        telex_types::Backing::Shared(s) => s.as_bytes().to_vec(),
    }
}
