...
use telex_shm::{Segment, SegmentConfig, ShmLink, create_test_link_pair};
use telex_types::{
...
impl Handler<DriverReplySink> for BlobEchoHandler {
    async fn handle(
        ...
        let blob: Vec<u8> = telex_postcard::from_slice(args_bytes).expect("deserialize blob");
        eprintln!("[blob handler] got blob len={}, sending back", blob.len());
        let response_bytes = telex_postcard::to_vec(&blob).unwrap();
        eprintln!(
...
async fn echo_blob_stress_over_shm_link() {
    ...
    for i in 0..200 {
        ...
        let echoed: Vec<u8> = telex_postcard::from_slice(ret_bytes).unwrap_or_else(|e| {
            panic!(
