...
use telex_types::{
    ...
    RequestCancel, RequestMessage, RequestResponse, RetryPolicy, TelexError, SelfRef, Tx, WriteSlot,
    channel, ensure_operation_id, metadata_operation_id,
};

use telex_types::{HandshakeResult, SessionResumeKey, SessionRole};
...
impl Handler<DriverReplySink> for EchoHandler {
    async fn handle(
        ...
        let result: u32 = telex_postcard::from_slice(args_bytes).expect("deserialize args");
        reply
...
impl OperationStore for CountingOperationStore {
    fn admit(&self, operation_id: telex_types::OperationId) {
        self.admits.fetch_add(1, Ordering::SeqCst);
        self.inner.admit(operation_id)
    }

    fn lookup(&self, operation_id: telex_types::OperationId) -> crate::OperationState {
        self.inner.lookup(operation_id)
    }

    fn get_sealed(&self, operation_id: telex_types::OperationId) -> Option<crate::SealedResponse> {
        self.inner.get_sealed(operation_id)
    }

    fn seal(
        &self,
        operation_id: telex_types::OperationId,
        response: &telex_types::PostcardPayload,
    ...
    fn remove(&self, operation_id: telex_types::OperationId) {
        self.inner.remove(operation_id)
    }

    fn schema_source(&self) -> &dyn telex_types::SchemaSource {
...
async fn dropping_one_root_caller_clone_keeps_session_alive_until_last_drop() {
    ...
    let echoed: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize response");
    assert_eq!(echoed, 42);
...
async fn dropping_root_caller_waits_for_virtual_connections_before_session_shutdown() {
    ...
    impl ConnectionAcceptor for LocalEchoAcceptor {
        fn accept(
            &self,
            _conn_id: telex_types::ConnectionId,
            peer_settings: &ConnectionSettings,
            _metadata: &[telex_types::MetadataEntry],
...
async fn dropping_root_caller_keeps_session_alive_while_bound_stream_rx_exists() {
    ...
    let _bytes = telex_types::channel::with_channel_binder(&root_caller, || {
        telex_postcard::to_vec(&args).expect("serialize args")
    });
...
async fn cancel_aborts_in_flight_handler() {
    ...
    let cancel_req_id = telex_types::RequestId(1);
    client_sender
    ...
    let error: Result<(), TelexError> =
        telex_postcard::from_slice(ret_bytes).expect("deserialize response");
    assert!(
        matches!(error, Err(TelexError::Cancelled)),
        "expected Err(TelexError::Cancelled) in response payload"
    );
...
async fn cancel_does_not_abort_persist_handler() {
    ...
    let value: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize response");
    assert_eq!(value, 123);
}
...
async fn caller_injects_operation_id_when_peer_supports_retry() {
    ...
    let operation_id: u64 = telex_postcard::from_slice(ret_bytes).expect("deserialize response");
...
async fn operation_replay_after_resume_delivers_sealed_outcome() {
    ...
    ensure_operation_id(&mut metadata, telex_types::OperationId(99));
    ...
    let value: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize");
    assert_eq!(value, 11);
...
fn message_plan_from_identical_schemas_round_trips() {
    let schemas = telex_types::extract_schemas(<Message<'static> as Facet<'static>>::SHAPE)
    ...
    let msg = Message {
        connection_id: telex_types::ConnectionId::ROOT,
        payload: MessagePayload::Ping(telex_types::Ping { nonce: 42 }),
    };
    let bytes = telex_postcard::to_vec(&msg).expect("serialize message");
    ...
    assert_eq!(decoded.connection_id, telex_types::ConnectionId::ROOT);
...
async fn resumable_session_returns_indeterminate_for_released_non_idem_call_after_manual_resume() {
    ...
    let result: Result<u32, TelexError> =
        telex_postcard::from_slice(ret_bytes).expect("deserialize response");
    assert!(matches!(result, Err(TelexError::Indeterminate)));
...
async fn in_flight_call_returns_cancelled_when_peer_closes() {
    ...
    assert!(
        matches!(
            result,
            Err(TelexError::ConnectionClosed) | Err(TelexError::SessionShutdown)
        ),
...
async fn open_virtual_connection_and_call() {
    ...
    let result: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize response");
...
async fn schema_tracker_is_per_connection_not_per_session() {
    ...
    let result: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize root response");
    ...
    let result: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize vconn response");
...
async fn close_root_connection_is_rejected() {
    ...
    let result = session_handle
        .close_connection(telex_types::ConnectionId::ROOT, vec![])
        .await;
    assert!(
...
async fn close_unknown_virtual_connection_is_rejected() {
    ...
    let missing_conn_id = telex_types::ConnectionId(1);
    let result = session_handle
...
async fn close_virtual_connection() {
    ...
    let result: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize");
    assert_eq!(result, 42);
...
async fn buffers_inbound_channel_items_until_rx_is_registered() {
    ...
    let decoded: u32 = telex_postcard::from_slice(bytes).expect("deserialize buffered item");
...
async fn proxy_connections_forwards_calls_without_service_specific_proxy_code() {
    ...
    let result: u32 = telex_postcard::from_slice(ret_bytes).expect("deserialize proxied response");
