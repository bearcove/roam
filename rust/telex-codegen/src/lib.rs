...
//! Code generation for telex RPC bindings across multiple languages.
//!
//! # This Is Where Code Generation Actually Happens
//!
//! While [`telex-macros`] parses your service traits and emits metadata, this crate
...
//!     let ts_code = telex_codegen::targets::typescript::generate(&detail);
//!     std::fs::write("generated/calculator.ts", ts_code).unwrap();
//!
//!     // Generate Go server
//!     let go_code = telex_codegen::targets::go::generate(&detail);
...
//!
//! ```text
//! #[service] trait     →    ServiceDescriptor    →    telex-codegen    →    .ts, .go, .swift, ...
...
//! no idea if `Tx` refers to `telex::channel::Tx` or some user-defined type. It cannot
...
//!
//! ```ignore
//! // We can identify telex's Tx vs user-defined types
...
//! - Is this actually telex's `Tx`/`Rx` channel type?
...
use telex_types::MethodDescriptor;
