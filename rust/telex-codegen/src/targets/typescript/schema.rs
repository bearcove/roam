...
use telex_types::{
    TelexError, Schema, SchemaHash, SchemaKind, ServiceDescriptor, ShapeKind, TypeRef,
...
///
/// The response schema is ALWAYS wrapped as `Result<T, TelexError<E>>` to match
...
pub fn generate_send_schema_table(service: &ServiceDescriptor) -> String {
    ...
    fn extract_into(shape: &'static Shape, all_schemas: &mut Vec<Schema>) -> TypeRef<SchemaHash> {
        let extracted = telex_types::extract_schemas(shape).expect("schema extraction");
    ...
    let telex_error_template_root = extract_into(
        <TelexError<std::convert::Infallible> as Facet<'static>>::SHAPE,
        &mut all_schemas,
    );
    let telex_error_type_id = type_id_of(&telex_error_template_root);
    ...
    for method in service.methods {
        ...
        // --- Response ---
        // The wire encoding is ALWAYS Result<T, TelexError<E>>.
        ...
        let telex_error_ref = TypeRef::generic(telex_error_type_id, vec![err_ref]);

        method_schema_infos.push(MethodSchemaInfo {
            method_id,
            args_root,
            response_root: TypeRef::generic(result_type_id, vec![ok_ref, telex_error_ref]),
        });
    }
    ...
    out.push_str(&format!(
        "export const {service_name_lower}_send_schemas: import(\"@bearcove/telex-core\").ServiceSendSchemas = {{\n"
    ));

    // schemas: Map<bigint, Schema>
    out.push_str("  schemas: new Map<bigint, import(\"@bearcove/telex-postcard\").Schema>([\n");
    ...
    out.push_str(
        "  methods: new Map<bigint, import(\"@bearcove/telex-core\").MethodSendSchemas>([\n",
    );
...
use telex_types::{ChannelDirection, FieldSchema, PrimitiveType};
