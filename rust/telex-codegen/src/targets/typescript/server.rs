...
use telex_types::{ServiceDescriptor, ShapeKind, classify_shape};
...
///
/// Implements `Dispatcher` from telex-core:
...
pub fn generate_dispatcher_class(service: &ServiceDescriptor) -> String {
    ...
    // dispatch()
    out.push_str(
        "  async dispatch(_context: RequestContext, method: MethodDescriptor, args: unknown[], call: TelexCall): Promise<void> {\n",
    );
