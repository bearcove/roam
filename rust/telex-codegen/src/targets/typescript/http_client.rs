...
//!
//! Generates a fetch()-based HTTP client for calling telex services via the HTTP bridge.
...
use telex_types::{ServiceDescriptor, is_rx, is_tx};
...
pub fn generate_http_client(service: &ServiceDescriptor) -> String {
    ...
    // Interface
    out.push_str(&format!(
        "// HTTP client for {service_name} (via telex HTTP bridge)\n"
    ));
