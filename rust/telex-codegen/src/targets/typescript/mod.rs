//! TypeScript code generation for telex services.
//!
...
use telex_types::{MethodDescriptor, ServiceDescriptor};
...
pub fn generate_method_ids(methods: &[&MethodDescriptor]) -> String {
    ...
    out.push_str("// @generated by telex-codegen\n");
    out.push_str("// This file defines canonical telex method IDs.\n\n");
...
pub fn generate_service(service: &ServiceDescriptor) -> String {
    ...
    // Header
    cw_writeln!(w, "// @generated by telex-codegen").unwrap();
    cw_writeln!(
        w,
...
fn generate_imports(service: &ServiceDescriptor, w: &mut CodeWriter<&mut String>) {
    use crate::cw_writeln;
    use telex_types::{ShapeKind, classify_shape, is_rx, is_tx};
    ...
    cw_writeln!(
        w,
        "import type {{ Caller, MethodDescriptor, ServiceDescriptor, TelexCall, Dispatcher, RequestContext, SessionTransportOptions }} from \"@bearcove/telex-core\";"
    )
    .unwrap();
    cw_writeln!(w, "import {{ session }} from \"@bearcove/telex-core\";").unwrap();

    // WebSocket transport for connect helper
    cw_writeln!(w, "import {{ wsConnector }} from \"@bearcove/telex-ws\";").unwrap();

    // RpcError for fallible client methods
    if has_fallible {
        cw_writeln!(w, "import {{ RpcError }} from \"@bearcove/telex-core\";").unwrap();
    }
    ...
    if has_streaming {
        cw_writeln!(
            w,
            "import {{ Tx, Rx, argElementRefsForMethod, bindChannelsForTypeRefs, finalizeBoundChannelsForTypeRefs }} from \"@bearcove/telex-core\";"
        )
        .unwrap();
    }
}
...
mod tests {
    ...
    fn generated_typescript_contains_no_postcard_primitive_usage() {
        ...
        assert!(
            !generated.contains("import * as pc from \"@bearcove/telex-postcard\""),
