...
use telex_types::{
    EnumInfo, ServiceDescriptor, ShapeKind, StructInfo, TelexError, TypeRef, VariantKind,
...
fn generate_wire_schemas(service: &ServiceDescriptor) -> String {
    ...
    use telex_types::{Schema, SchemaHash};
    ...
    for method in service.methods {
        ...
        // Collect response schema IDs (including Result and TelexError)
...
fn format_swift_schema(schema: &telex_types::Schema) -> String {
    use crate::render::hex_u64;
...
fn format_swift_schema_kind(kind: &telex_types::SchemaKind) -> String {
    use telex_types::SchemaKind;
    ...
    match kind {
        ...
        SchemaKind::Channel { direction, element } => {
            let dir = match direction {
                telex_types::ChannelDirection::Tx => ".tx",
                telex_types::ChannelDirection::Rx => ".rx",
            };
            format!(
...
fn format_swift_variant_payload(payload: &telex_types::VariantPayload) -> String {
    use telex_types::VariantPayload;
...
fn format_swift_primitive(prim: &telex_types::PrimitiveType) -> String {
    use telex_types::PrimitiveType;
