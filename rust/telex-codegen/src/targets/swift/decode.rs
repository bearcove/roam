...
pub fn generate_decode_stmt_from_with_cursor(
    ...
    match classify_shape(shape) {
        ...
        ShapeKind::Enum(EnumInfo {
            ...
            out.push_str(&format!(
                "{indent}    throw TelexError.decodeError(\"unknown enum variant\")\n"
            ));
        ...
        ShapeKind::Result { ok, err } => {
            ...
            out.push_str(&format!("{indent}default:\n"));
            out.push_str(&format!(
                "{indent}    throw TelexError.decodeError(\"invalid Result discriminant\")\n"
            ));
            out.push_str(&format!("{indent}}}\n"));
            out
        }
...
pub fn generate_decode_closure(shape: &'static Shape) -> String {
    ...
    match classify_shape(shape) {
        ...
        ShapeKind::Enum(EnumInfo {
            ...
            code.push_str("    default:\n        throw TelexError.decodeError(\"unknown enum variant\")\n    }\n    return result\n}");
            code
        }
        ShapeKind::Pointer { pointee } => generate_decode_closure(pointee),
        _ => "{ _, _ in throw TelexError.decodeError(\"unsupported type\") }".into(),
    }
}
