#include "telex_shm_atomic.h"

#include <errno.h>
#include <string.h>
#include <sys/socket.h>

int telex_recv_one_fd(int sockfd, int *out_fd) {
  int fds[1];
  int rc = telex_recv_fds(sockfd, fds, 1);
  if (rc <= 0) {
    return rc;
  }
  *out_fd = fds[0];
...
int telex_recv_fds(int sockfd, int *out_fds, int max_fds) {
...
int telex_send_fds(int sockfd, const int *fds, int num_fds) {
