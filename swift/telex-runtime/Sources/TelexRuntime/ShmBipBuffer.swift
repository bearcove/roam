...
public final class ShmBipBuffer: @unchecked Sendable {
    ...
    public static func initialize(region: ShmRegion, headerOffset: Int, capacity: UInt32) throws -> ShmBipBuffer {
        ...
        telex_bipbuf_init(headerRaw, capacity)
    ...
    public static func attach(region: ShmRegion, headerOffset: Int) throws -> ShmBipBuffer {
        ...
        let headerRaw = try region.pointer(at: headerOffset)
        let capacity = telex_bipbuf_capacity(headerRaw)
        guard capacity > 0 else {
            throw ShmBipBufferError.invalidCapacity
        }
    ...
    func debugState() -> (read: UInt32, write: UInt32, watermark: UInt32) {
        (
            read: telex_bipbuf_load_read_acquire(headerPointer),
            write: telex_bipbuf_load_write_acquire(headerPointer),
            watermark: telex_bipbuf_load_watermark_acquire(headerPointer)
        )
    }
    ...
    public func tryGrant(_ len: UInt32) throws -> UnsafeMutableRawBufferPointer? {
        ...
        var offset: UInt32 = 0
        let result = telex_bipbuf_try_grant(headerPointer, len, &offset)
        if result < 0 {
            throw ShmBipBufferError.grantTooLarge
        }
        if result == 0 {
    ...
    public func commit(_ len: UInt32) throws {
        if telex_bipbuf_commit(headerPointer, len) != 0 {
    ...
    public func tryRead() -> UnsafeRawBufferPointer? {
        var offset: UInt32 = 0
        var length: UInt32 = 0
        let result = telex_bipbuf_try_read(headerPointer, &offset, &length)
        if result == 1 {
    ...
    public func release(_ len: UInt32) throws {
        if telex_bipbuf_release(headerPointer, len) != 0 {
            throw ShmBipBufferError.releaseOverflow
        }
    }

    public func isEmpty() -> Bool {
        let read = telex_bipbuf_load_read_acquire(headerPointer)
        let write = telex_bipbuf_load_write_acquire(headerPointer)
        let watermark = telex_bipbuf_load_watermark_acquire(headerPointer)
