...
private let traceToStderr = ProcessInfo.processInfo.environment["TELEX_TRACE_STDERR"] != nil
private let traceFilePath = ProcessInfo.processInfo.environment["TELEX_TRACE_FILE"]
private let subsystem = "bearcove.telex.runtime"
...
private func parseTraceCategories() -> Set<TraceCategory> {
    guard let raw = ProcessInfo.processInfo.environment["TELEX_TRACE"] else {
        return []
    }
    let tokens = raw
        .split(separator: ",")
