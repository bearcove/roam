...
public final class ShmVarSlotPool: @unchecked Sendable {
    ...
    public init(region: ShmRegion, baseOffset: Int, classes: [ShmVarSlotClass]) throws {
        ...
        let ffiClasses = classes.map { TelexSizeClass(slot_size: $0.slotSize, count: $0.count) }
        guard let pool = ffiClasses.withUnsafeBufferPointer({ buf in
            telex_var_slot_pool_attach(
    ...
    public func initialize() {
        telex_var_slot_pool_init(pool)
    }

    // r[impl shm.varslot.allocate]
    public func alloc(size: UInt32, owner: UInt8) -> ShmVarSlotHandle? {
        var out = TelexVarSlotHandle(class_idx: 0, extent_idx: 0, slot_idx: 0, generation: 0)
        let result = telex_var_slot_pool_alloc(pool, size, owner, &out)
        if result == 1 {
    ...
    public func markInFlight(_ handle: ShmVarSlotHandle) throws {
        let result = telex_var_slot_pool_mark_in_flight(pool, toFfi(handle))
        if result != 0 {
    ...
    public func free(_ handle: ShmVarSlotHandle) throws {
        let result = telex_var_slot_pool_free(pool, toFfi(handle))
        if result != 0 {
            throw ShmVarSlotFreeError.ffiError
        }
    }
    ...
    public func freeAllocated(_ handle: ShmVarSlotHandle) throws {
        let result = telex_var_slot_pool_free_allocated(pool, toFfi(handle))
        if result != 0 {
            throw ShmVarSlotFreeError.ffiError
        }
    }
    ...
    public func slotSize(classIdx: UInt8) -> UInt32? {
        let size = telex_var_slot_pool_slot_size(pool, classIdx)
        return size > 0 ? size : nil
    }

    public func payloadPointer(_ handle: ShmVarSlotHandle) -> UnsafeMutableRawPointer? {
        guard let ptr = telex_var_slot_pool_payload_ptr(pool, toFfi(handle)) else {
            return nil
        }
    ...
    public func slotState(_ handle: ShmVarSlotHandle) -> ShmSlotState {
        let raw = telex_var_slot_pool_slot_state(pool, toFfi(handle))
        return ShmSlotState(rawValue: UInt32(max(raw, 0))) ?? .free
    }

    public func recoverPeer(ownerPeer: UInt8) {
        telex_var_slot_pool_recover_peer(pool, ownerPeer)
    }

    @inline(__always)
    private func toFfi(_ handle: ShmVarSlotHandle) -> TelexVarSlotHandle {
        TelexVarSlotHandle(
...
public final class ShmMmapAttachments: @unchecked Sendable {
    ...
    public init?(controlFd: Int32) {
        guard controlFd >= 0, let raw = telex_mmap_attachments_create(controlFd) else {
            return nil
        }
        self.raw = raw
    }
    ...
    deinit {
        telex_mmap_attachments_destroy(raw)
    }

    public func drainControl() -> Bool {
        telex_mmap_attachments_drain_control(raw) >= 0
    }

    public func resolve(mmapRef: ShmMmapRef) -> [UInt8]? {
        var ptr: UnsafePointer<UInt8>?
        let rc = telex_mmap_attachments_resolve_ptr(
            raw,
            mmapRef.mapId,
