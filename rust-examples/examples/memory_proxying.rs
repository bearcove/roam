...
async fn run_demo() -> Result<()> {
    let (host_a_link, guest_a_link) = telex::memory_link_pair(64);
    let (host_b_link, guest_b_link) = telex::memory_link_pair(64);

    println!("[guest-b] starting root session");
    let guest_b_task = tokio::spawn(async move {
        let (guest_b_root_guard, _) = telex::acceptor_on_link(
            guest_b_link,
            ConnectionSettings {
                parity: Parity::Even,
                max_concurrent_requests: 64,
    ...
    let (_host_root_to_b_guard, upstream_session_handle) = telex::initiator_on_link(
        host_b_link,
        ConnectionSettings {
            parity: Parity::Odd,
            max_concurrent_requests: 64,
    ...
    let host_for_a_task = tokio::spawn(async move {
        let (host_root_for_a_guard, _) = telex::acceptor_on_link(
            host_a_link,
            ConnectionSettings {
                parity: Parity::Even,
                max_concurrent_requests: 64,
    ...
    let (_guest_a_root_guard, guest_a_session_handle) = telex::initiator_on_link(
        guest_a_link,
        ConnectionSettings {
            parity: Parity::Odd,
            max_concurrent_requests: 64,
