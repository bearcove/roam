...
impl WordLab for WordLabService {
    ...
    async fn classify<'telex>(
        &self,
        call: impl Call<'telex, &'telex str, Infallible>,
        word: String,
    ) {
...
async fn run_demo() -> Result<()> {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let (server_caller_guard, _) = telex::acceptor_on(StreamLink::tcp(socket))
    ...
    let (client, _) = telex::initiator_on(StreamLink::tcp(socket), telex::TransportMode::Bare)
