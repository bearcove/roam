...
impl LinkTx for KillableMemoryLinkTx {
    type Permit = telex::MemoryLinkTxPermit;
...
fn killable_memory_link_pair(
    ...
    let (a, b) = telex::memory_link_pair(buffer);
...
where
    L: Link + Send + 'static,
{
    type Link = telex::SplitLink<L::Tx, L::Rx>;
...
async fn run_demo() -> Result<()> {
    ...
    println!("[demo] establishing telex session over stable conduits");
    let server_task = tokio::spawn(async move {
        let server_settings = ConnectionSettings {
            parity: Parity::Even,
            max_concurrent_requests: 64,
        };
        let (server_guard, _) = telex::acceptor_conduit(
            server_conduit,
            HandshakeResult {
    ...
    let (client, _) = telex::initiator_conduit(
        client_conduit,
        HandshakeResult {
