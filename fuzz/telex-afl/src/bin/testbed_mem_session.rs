...
use afl::fuzz;
use telex::Call;
use telex_core::{BareConduit, DriverReplySink, acceptor, initiator, memory_link_pair};
use telex_types::{Handler, MessageFamily, RequestCall, SelfRef};
use spec_proto::{
...
impl Testbed for FuzzService {
    ...
    async fn sum(&self, call: impl Call<i64, Infallible>, mut numbers: telex::Rx<i32>) {
        let mut total: i64 = 0;
    ...
    async fn generate(&self, call: impl Call<(), Infallible>, count: u32, output: telex::Tx<i32>) {
    ...
    async fn transform(
        ...
        mut input: telex::Rx<String>,
        output: telex::Tx<String>,
    ) {
...
async fn run_case(data: &[u8]) {
    ...
    for _ in 0..ops {
        match cur.next_u8() % 10 {
            ...
            7 => {
                let (in_tx, in_rx) = telex::channel::<String>();
                let (out_tx, mut out_rx) = telex::channel::<String>();
