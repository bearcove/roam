...
This section defines Rust implementation layering, bottom-up.
...
> rs[layer.transport]
>
> The transport layer is responsible for moving roam `Message` values between
> peers and for transport-specific framing/signaling.

> rs[layer.transport.stream]
>
> Stream transports (for example: TCP, Unix sockets) MUST frame roam messages
> with a 4-byte little-endian
> length prefix (`r[transport.bytestream.length-prefix]`), and decode back to
> discrete roam `Message` values before protocol processing.
>
> Each framed message payload MUST be the postcard encoding of one roam
> `Message`.

> rs[layer.transport.framed]
>
> Framed transports (for example: WebSocket) MUST map one transport
> frame/message to one roam `Message` (`r[transport.message.one-to-one]`) and
> preserve message boundaries.
>
> Each frame/message payload MUST be the postcard encoding of one roam
> `Message`.

> rs[layer.transport.shm]
>
> SHM transport uses shared-memory buffers and signaling to exchange roam
> `Message` values. Despite different mechanics, it MUST preserve the same
> link/virtual-connection/channel semantics as other transports.

## Link Runtime

> rs[layer.link-runtime]
>
> The Rust link runtime owns virtual-connection call execution:
...
> impls. They own method descriptors, typed argument construction, and an inner
> call-capable runtime handle; they MUST NOT own link I/O, reconnect/retry
