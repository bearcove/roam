+++
title = "Introduction"
description = "Formal telex RPC protocol specification"
weight = 10
+++

> r[service-macro.is-source-of-truth]
>
> telex is a **Rust-native** RPC protocol. There is no independent schema language;
...
possible, modulo orphan rules) so that they may be joined with telex codegen to
...
Given an `Adder` trait, the `telex::service` proc macro generates an
`Adder` trait:
...
telex supports various transports, like memory, TCP and other sockets, WebSocket,
shared memory; but a telex connection sits several layers above a "TCP connection".
...
telex uses several related terms that refer to different layers of the system.
