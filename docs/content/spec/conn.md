...
> r[transport.memory]
>
> Telex provides an in-memory transport via `MemoryLink`, based on tokio MPSC
> channels.

> r[transport.stream]
>
> Telex provides a stream transport via `StreamLink`, which prefixes each payload
...
>
> Telex provides a `LocalLink` abstraction that uses named pipes on Windows and
> Unix sockets on Linux & macOS. Endpoints/addresses are a `String` internally.

> r[transport.websocket]
>
> Telex provides a WebSocket link, which sends payloads via WebSocket binary
> frames.
...
>
> Telex provides an in-process link for WASM ↔ JS communication within the
...
> r[transport.shm]
>
> Telex provides a shared memory transport. It is designed for high-performance
...
>
> After `TransportReject`, the link attachment is unusable for telex traffic and
...
>
> When their counterpart does something that violates the telex spec, a peer MUST
...
generator. It's using telex RPC over the shared memory transport to communicate
...
accepts new telex sessions over WebSocket connections, to serve the DevTools
...
The Rust runtime (`telex-core`) exposes virtual connections as first-class
session operations:
...
```rust
let (mut session, root_handle, session_handle) = telex_core::session::initiator(conduit)
    .establish()
    .await?;

let mut root_driver = telex_core::Driver::new(root_handle, root_dispatcher, telex_types::Parity::Odd);
...
let mut vconn_driver = telex_core::Driver::new(vconn_handle, vconn_dispatcher, telex_types::Parity::Odd);
...
```rust
let (mut session, root_handle, _session_handle) = telex_core::session::acceptor(conduit)
