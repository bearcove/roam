+++
title = "Rust Guide"
description = "Use telex + a transport crate to define services, run drivers, and call methods with channels."
weight = 21
+++
...
- Source: [rust-examples/examples/borrowed_and_channels.rs](https://github.com/bearcove/telex/blob/main/rust-examples/examples/borrowed_and_channels.rs)
...
- Source: [rust-examples/examples/virtual_connections.rs](https://github.com/bearcove/telex/blob/main/rust-examples/examples/virtual_connections.rs)
...
- Source: [rust-examples/examples/stable_conduit_reconnect.rs](https://github.com/bearcove/telex/blob/main/rust-examples/examples/stable_conduit_reconnect.rs)
...
- Source: [rust-examples/examples/memory_proxying.rs](https://github.com/bearcove/telex/blob/main/rust-examples/examples/memory_proxying.rs)
...
> ```rust
> telex::proxy_connections(incoming_handle, upstream_conn).await;
> ```

## 5) `shm_host_two_guests` (most complex: host + multiple guest processes)

- Source: [rust-examples/examples/shm_host_two_guests.rs](https://github.com/bearcove/telex/blob/main/rust-examples/examples/shm_host_two_guests.rs)
...
Most application code only needs `telex` + one transport crate.
...
Define a service with `#[telex::service]`, implement it, and establish on each side:

```rust
let (server_guard, _) = telex::acceptor(StreamLink::tcp(server_socket))
    .establish::<WordLabClient>(WordLabDispatcher::new(WordLabService))
    .await?;

let (client, _session_handle) = telex::initiator(StreamLink::tcp(client_socket))
...
```rust
async fn classify<'telex>(
    &self,
    call: impl telex::Call<'telex, &'telex str, std::convert::Infallible>,
    word: String,
) {
    call.ok("short").await;
}
...
For non-Rust bindings, generate code from service descriptors with `telex-codegen`.
