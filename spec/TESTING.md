...
This document describes the testing infrastructure for telex protocol conformance.

## Overview

The spec-tests crate provides a **protocol conformance test suite** that validates
telex implementations at the wire level. The goal is to test implementations as
black boxes, communicating only via the telex wire protocol.
...
The harness uses `LengthPrefixedFramed` to send/receive raw wire messages. No telex
runtime types are used - just `telex_wire::Message`.
...
- Unknown method → `TelexError::UnknownMethod`
- Invalid payload → `TelexError::InvalidPayload`
...
**How it works:**
```
[Test Harness as Server] <----telex protocol----> [Subject as Client]
```

**Note:** The harness must implement the `Testbed` service to act as server.
This currently uses telex runtime types (`Rx`, `Tx`, `ServiceDispatcher`).
...
| `TELEX_WIRE_SPY` | Enable wire-level message logging | — |
...
### Issue: Harness Using telex Internals

Currently, `client_mode.rs` uses telex runtime types (`Rx`, `Tx`,
`telex_stream::establish_acceptor`) to act as a server. This is problematic
because:

1. We're testing telex-against-telex rather than testing the wire protocol
