...
pub fn run_rpc_user_error_roundtrip(spec: SubjectSpec) {
    run_async(async {
        ...
        match result {
            Err(TelexError::User(MathError::DivisionByZero)) => {}
            Ok(resp) => {
                return Err(format!(
...
pub fn run_rpc_divide_overflow(spec: SubjectSpec) {
    run_async(async {
        ...
        match result {
            Err(TelexError::User(MathError::Overflow)) => {}
...
pub fn run_rpc_lookup_access_denied(spec: SubjectSpec) {
    run_async(async {
        ...
        match result {
            Err(TelexError::User(LookupError::AccessDenied)) => {}
