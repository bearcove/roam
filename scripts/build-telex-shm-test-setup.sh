#!/usr/bin/env bash
set -euo pipefail

cargo build -p telex-shm --bin guest_process --all-features

# Skip Swift builds on Linux (Swift is only available on macOS)
if [[ "$OSTYPE" == "darwin"* ]]; then
  # Build the Rust FFI staticlib that Swift links against
  cargo build --release -p telex-shm-ffi
  swift build --package-path swift/telex-runtime --product shm-bootstrap-client
  swift build --package-path swift/telex-runtime --product shm-guest-client
  swift build -c release --package-path swift/subject --product subject-swift
else
  echo "Skipping Swift builds on non-macOS platform ($OSTYPE)"
fi
