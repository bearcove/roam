...
const CLIENT_HELLO_MAGIC = new Uint8Array([0x54, 0x58, 0x43, 0x48]); // TXCH
const SERVER_HELLO_MAGIC = new Uint8Array([0x54, 0x58, 0x53, 0x48]); // TXSH
  ...
  async send(item: Message): Promise<void> {
    ...
    while (!this.closed) {
      try {
        ...
        await link.send(encodeStableFrame(frame));
        return;
      } catch {
    ...
    this.recvLoopPromise = (async () => {
      while (!this.closed) {
        try {
          ...
          const decoded = decodeStableFrame(payload, 0);
