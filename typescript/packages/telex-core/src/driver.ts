...
class TelexCallImpl implements TelexCall {
...
export class Driver {
      ...
      switch (message.kind) {
        case "data":
          await this.connection.sendChannelData(message.channelId, message.payload).catch((error) => {
            telexLogger()?.error("[telex:driver] failed to send channel data", error);
          });
          break;
        case "close":
          await this.connection.sendChannelClose(message.channelId).catch((error) => {
            telexLogger()?.error("[telex:driver] failed to send channel close", error);
          });
          break;
        case "grantCredit":
          await this.connection.sendChannelCredit(message.channelId, message.additional).catch((error) => {
            telexLogger()?.error("[telex:driver] failed to grant channel credit", error);
          });
          break;
        case "response":
          await this.connection
            ...
            .catch((error) => {
              telexLogger()?.error("[telex:driver] failed to send response", error);
            });
          break;
      }
    }
  ...
  private async handleCall(incoming: IncomingCall): Promise<void> {
    ...
    telexLogger()?.debug(`[telex:driver] handleCall: methodId=${incoming.methodId} method=${method?.name ?? "UNKNOWN"}`);
    if (!method) {
      telexLogger()?.debug(`[telex:driver] unknown method, sending error response`);
    ...
    try {
      ...
      telexLogger()?.debug(`[telex:driver] dispatching ${method.name} with ${args.length} args`);
      await this.dispatcher.dispatch(context, method, args, call);
      telexLogger()?.debug(`[telex:driver] dispatch complete for ${method.name}, didReply=${call.didReply()}`);
      ...
      telexLogger()?.error(`[telex:driver] dispatch error for ${method.name}:`, error);
      if (!call.didReply()) {
