// Logging middleware for telex clients.
//
...
import { RpcError, RpcErrorCode } from "@bearcove/telex-wire";
...
export interface LoggingOptions {
  /**
   * Namespace for debug matching. Defaults to "telex:rpc".
   * Logging is enabled when localStorage.debug matches this namespace.
   * Supports patterns like "telex:*" or "*".
   */
  namespace?: string;
 ...
 * ```javascript
 * localStorage.debug = 'telex:*'  // Enable all telex logging
 * localStorage.debug = 'telex:rpc'  // Enable only RPC logging
...
export function loggingMiddleware(options: LoggingOptions = {}): ClientMiddleware {
  const namespace = options.namespace ?? "telex:rpc";
