...
import { buildPlan, resolveTypeRef } from "@bearcove/telex-postcard";
...
export class SchemaTracker {
  ...
  recordReceived(
    ...
    telexLogger()?.debug(
      `[telex:schema] recorded ${payload.schemas.length} schemas for method=${methodId} direction=${direction} root=${debugJson(payload.root)} resolved=${resolvedRootKind?.tag ?? "missing"}`,
    );
  }
  ...
  buildTranslation(
    ...
    telexLogger()?.debug(
      `[telex:schema] buildTranslation method=${methodId} direction=${direction} remote=${rootKind.tag} local=${localSchemaSet.root.kind.tag} root=${debugJson(binding.rootRef)}`,
    );
...
export interface MethodSendSchemas {
  ...
  argsRootRef: import("@bearcove/telex-postcard").TypeRef;
  /** Root TypeRef for the response schema. */
  responseRootRef: import("@bearcove/telex-postcard").TypeRef;
}

/** Pre-computed send schema data for a service (generated by telex-codegen). */
export interface ServiceSendSchemas {
  /** All type schemas used by this service. typeId (content hash) → Schema object. */
  schemas: Map<bigint, import("@bearcove/telex-postcard").Schema>;
...
export class SchemaSendTracker {
  ...
  prepareSchemas(
    ...
    const schemas = newDeps
      ...
      .filter((s): s is import("@bearcove/telex-postcard").Schema => s !== undefined);
