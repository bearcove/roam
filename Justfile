...
swift-subject-cov *args:
    ...
    LLVM_PROFILE_FILE="$(pwd)/.coverage/swift-subject/runtime-tests-%p%c.profraw" \
      swift test --package-path swift/telex-runtime --no-parallel \
        -Xswiftc -profile-generate \
        -Xswiftc -profile-coverage-mapping
    RUNTIME_TEST_BIN="$(swift build --package-path swift/telex-runtime --show-bin-path)/telex-runtimePackageTests.xctest/Contents/MacOS/telex-runtimePackageTests" && \
...
fuzz-targets:
    ...
    @echo "  framing_peek         (fuzz/telex-shm-afl)"
    @echo "  shm_link_roundtrip   (fuzz/telex-shm-afl)"
    @echo "  protocol_decode      (fuzz/telex-afl)"
    @echo "  testbed_mem_session  (fuzz/telex-afl)"
    @echo ""
...
fuzz-build target="all":
    @case "{{ target }}" in \
      ...
      framing_peek|shm_link_roundtrip) \
        cargo afl build --manifest-path fuzz/telex-shm-afl/Cargo.toml --bin "{{ target }}"; \
        ;; \
      protocol_decode|testbed_mem_session) \
        cargo afl build --manifest-path fuzz/telex-afl/Cargo.toml --bin "{{ target }}"; \
        ;; \
      *) \
...
fuzz-run target="all" seconds="":
    ...
    @mkdir -p \
      fuzz/telex-shm-afl/out/framing_peek \
      fuzz/telex-shm-afl/out/shm_link_roundtrip \
      fuzz/telex-afl/out/protocol_decode \
      fuzz/telex-afl/out/testbed_mem_session
    ...
    case "{{ target }}" in \
      all) \
        run_fuzz "{{ seconds }}" fuzz/telex-shm-afl/in/framing_peek fuzz/telex-shm-afl/out/framing_peek fuzz/telex-shm-afl/target/debug/framing_peek; \
        run_fuzz "{{ seconds }}" fuzz/telex-shm-afl/in/shm_link_roundtrip fuzz/telex-shm-afl/out/shm_link_roundtrip fuzz/telex-shm-afl/target/debug/shm_link_roundtrip; \
        run_fuzz "{{ seconds }}" fuzz/telex-afl/in/protocol_decode fuzz/telex-afl/out/protocol_decode fuzz/telex-afl/target/debug/protocol_decode; \
        run_fuzz "{{ seconds }}" fuzz/telex-afl/in/testbed_mem_session fuzz/telex-afl/out/testbed_mem_session fuzz/telex-afl/target/debug/testbed_mem_session; \
        ;; \
      framing_peek) \
        run_fuzz "{{ seconds }}" fuzz/telex-shm-afl/in/framing_peek fuzz/telex-shm-afl/out/framing_peek fuzz/telex-shm-afl/target/debug/framing_peek; \
        ;; \
...
fuzz-sand-build target="all":
    @build_one() { \
      ...
      case "$t" in \
        framing_peek|shm_link_roundtrip) \
          manifest="fuzz/telex-shm-afl/Cargo.toml"; \
          bin_path="fuzz/telex-shm-afl/target/debug/$t"; \
          ;; \
        protocol_decode|testbed_mem_session) \
          manifest="fuzz/telex-afl/Cargo.toml"; \
          bin_path="fuzz/telex-afl/target/debug/$t"; \
          ;; \
        *) \
...
fuzz-sand-run target="all" seconds="":
    @run_one() { \
      t="$1"; \
      case "$t" in \
        framing_peek|shm_link_roundtrip) in_dir="fuzz/telex-shm-afl/in/$t" ;; \
        protocol_decode|testbed_mem_session) in_dir="fuzz/telex-afl/in/$t" ;; \
        *) \
          echo "Unknown target: $t" >&2; \
          just fuzz-targets; \
          exit 1; \
